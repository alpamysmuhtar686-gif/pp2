...
# created 29.03.2026, updated 23.04.2025 23:58
    ...
    try:
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cursor:
                ...
                # insert into phones table (explicit naming)
                cursor.execute("""
            ...
            conn.commit()
            print(f"[Success] Contact inserted with contact_id {contact_id}")
...
def add_extra_phone(contact_id, phone_number, phone_type):
    config = load_config()
    ...
    try:
        with psycopg2.connect(**config) as conn:
            ...
            conn.commit()
            print("[Success] Phone added successfully")

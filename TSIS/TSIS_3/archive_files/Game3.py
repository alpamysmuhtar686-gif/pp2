...
pygame.time.set_timer(SPAWN_EVENT, 2000)  # every 2 seconds, a new obstacle or event will be generated
...
while True:
    ...
    for pu in pygame.sprite.spritecollide(P1, powerups, True):
        ...
        if pu.type == "nitro":
            ...
            for obs in obstacles:
                if obs.rect.colliderect(P1.rect):
                    obs.kill()
                    break
    ...
    if P1.power:
        text = font_small.render(
            f"Power: {P1.power} ({P1.power_timer // 60}s)", 
            True, BLACK
        )
